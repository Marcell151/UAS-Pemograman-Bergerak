...
public class OrderAdapterCustomer extends RecyclerView.Adapter<OrderAdapterCustomer.OrderViewHolder> {
    ...
    public void onBindViewHolder(@NonNull OrderViewHolder holder, int position) {
        ...
        holder.tvStandName.setText("Stand ID: " + order.getStandId()); // Atau nama stand jika ada

        // 1. FORMAT RUPIAH
        holder.tvOrderTotal.setText(FormatHelper.formatRupiah(order.getTotal()));

        // 2. FORMAT WAKTU CANTIK
        if (holder.tvOrderDate != null) {
            String cantik = FormatHelper.formatWaktu(order.getCreatedAt());
            holder.tvOrderDate.setText("📅 " + cantik);
        }
